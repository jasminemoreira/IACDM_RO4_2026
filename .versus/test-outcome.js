...
// src/hooks/test-outcome.ts
var test_outcome_exports = {};
__export(test_outcome_exports, {
  detectExitCode: () => detectExitCode,
  detectOutcome: () => detectOutcome
});
module.exports = __toCommonJS(test_outcome_exports);
...
var CLEAN_MARKERS = [
  /\b0\s+(tests?\s+)?fail(ed|ures?|ing)?\b/i,
  /\bfail(ed|ures?)\s*[:=]\s*0\b/i,
  /\ball\s+tests?\s+passed\b/i,
  /\btest\s+result:\s*ok\b/i,
  // cargo
  /^ok\s|^PASS\b/im
  // go test / jest summary line
];
var PASS_MARKERS = [
...
var FAIL_MARKERS = [
  ...
  /\b[1-9]\d*\s+(tests?\s+)?fail(ed|ing|ures?)\b/i,
  /\bfail(ed|ures?)\s*[:=]\s*[1-9]\d*\b/i,
  /\bassertion(error| failed)\b/i,
  /\btest\s+result:\s*FAILED\b/i,
  /^FAIL\b/im,
  /\b(SyntaxError|ReferenceError|TypeError)\b/,
  /\bnpm\s+ERR!/i,
  /✗|✖|×/
];
function detectOutcome(output) {
  const clean = CLEAN_MARKERS.some((p) => p.test(output));
  const failed = FAIL_MARKERS.some((p) => p.test(output));
  if (failed && !clean) return "fail";
  if (failed && clean) {
    const hard = /\bexit\s+code:?\s*[1-9]\d*\b/i.test(output) || /^FAIL\b/im.test(output);
    return hard ? "fail" : "pass";
  }
  if (clean || PASS_MARKERS.some((p) => p.test(output))) return "pass";
  return "unknown";
}
function detectExitCode(output) {
  const ms = [...output.matchAll(/VERSUS_TEST_EXIT=(\d+)/g)];
  return ms.length ? parseInt(ms[ms.length - 1][1], 10) : void 0;
}
function emit(msg) {
  if (msg) {
...
function main() {
  ...
  process.stdin.on("end", () => {
    ...
    try {
      ...
      const output = responseText(resp);
      const harnessExit = resp && typeof resp === "object" && typeof resp.exit_code === "number" ? resp.exit_code : void 0;
      const echoExit = detectExitCode(output);
      const exitCode = typeof harnessExit === "number" ? harnessExit : echoExit;
      const exitSource = typeof harnessExit === "number" ? "harness" : typeof echoExit === "number" ? "echo" : void 0;
      const outcome = typeof exitCode === "number" ? exitCode === 0 ? "pass" : "fail" : detectOutcome(output);
      const provenance = {
        source: "harness",
        toolUseId: hookInput.tool_use_id,
        sessionId: hookInput.session_id,
        outputBytes: Buffer.byteLength(output, "utf-8"),
        outputSha256: (0, import_crypto2.createHash)("sha256").update(output).digest("hex"),
        exitCode,
        exitSource
      };
      const { count } = engine.recordTestOutcome(command, outcome, provenance);
      if (outcome === "fail" && count >= LOOP_FAILURE_THRESHOLD) {
        emit(
          `[Versus] S6: ${count} consecutive failed runs with no diagnosis recorded. The next test run will be BLOCKED until you name the cause via record_decision(phase=${state.currentPhase}, category='diagnosis', content='...'). If you already know why this failed, record it now and keep going.`
        );
        return;
      }
      if (outcome === "fail" && count === LOOP_FAILURE_THRESHOLD - 1) {
        emit(
          `[Versus] S6: ${count} failed runs in a row with no diagnosis recorded. Record the root cause of this failure with record_decision(category='diagnosis') before the next run, or S6 will block it.`
        );
        return;
      }
      emit();
      ...
      if (isTest) {
        try {
          engine.recordTestOutcome(command, "unknown", { source: "unknown" });
        } catch {
        }
      }
      emit();
