...
|---|---|---|---|
| 2026-08-19 | C | braço C seguiu 36 commits além de `inc3-fim`, **por decisão deliberada da operadora**, com ciclo novo aberto depois da fronteira do piloto: `0c3c583` congela um "requisito v2" próprio e o braço passa a construir **reprodução de áudio** (400 arquivos em duas vozes, cache dedicado, botões de voz), deck de 200 itens e arquitetura V(4) | **não é desvio de protocolo**: o piloto termina em `inc3-fim` e o que vem depois é uso posterior do workspace, não execução do estudo. Registrado apenas porque o escopo novo (áudio, que o §3 do `REQUISITOS.md` põe fora do enunciado) muda o estado final do repositório. **Efeito sobre o dado do piloto: nenhum**, desde que a medição pare na fronteira — os incrementos 1–3 estão marcados e foram medidos antes. Para garantir isso, `extrair.py` ganhou `--ate <ref>`, e toda medição do braço C no piloto usa `--ate inc3-fim`. Medir até HEAD misturaria 365 LOC fora do enunciado (1497 → 1862) no denominador |
