...
|---|---|---|---|
| 2026-08-19 | C | **braço C continuou 36 commits além de `inc3-fim`, fora do enunciado congelado**: `0c3c583` congela um "requisito v2" próprio e o braço passa a construir **reprodução de áudio** (400 arquivos em duas vozes, cache dedicado, botões de voz), deck de 200 itens e arquitetura V(4) | o §3 do `REQUISITOS.md` lista *"reprodução de áudio"* textualmente em **fora de escopo**, e proíbe incluí-la nos critérios de aceitação. **Efeito sobre o dado do piloto: nenhum**, desde que a medição pare na fronteira — os incrementos 1–3 estão marcados e foram medidos antes. Para garantir isso, `extrair.py` ganhou `--ate <ref>`, e toda medição do braço C no piloto usa `--ate inc3-fim`. Medir até HEAD misturaria 365 LOC fora do enunciado (1497 → 1862) no denominador |
